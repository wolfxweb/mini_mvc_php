...
class UsuarioModel extends Modelo{
    ...
    public static function getUsuarios($id = null){
      ...
      $stmSQL  = "select usu.usu_id, usu.usu_nome, usu.usu_email,usu_reset_token,  usu.sta_id, sta.sta_nome, sta.sta_sigla, usut.usut_id, usut.usut_nome, usut.usut_sigla
                    from usuarios usu
                        left  join status sta on sta.sta_id = usu.sta_id
                        left join usuario_tipo usut on usut.usut_id = usu.usut_id";
                        
       // $stmSQL  = "select * from usuarios "  ;              
        if($id){
